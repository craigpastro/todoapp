...
func (p *Postgres) Create(ctx context.Context, userID, data string) (string, time.Time, error) {
	postID := myid.New()
	now := time.Now()
	p.pool.Query(ctx, "INSERT INTO post VALUES ($1, $2, $3, $4, $5)", userID, postID, data, now, now)
...
func (p *Postgres) Read(ctx context.Context, userID, postID string) (*storage.Record, error) {
	...
	if err := row.Scan(&record.UserID, &record.PostID, &record.Data, &record.CreatedAt, &record.UpdatedAt); err == pgx.ErrNoRows {
