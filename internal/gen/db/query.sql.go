...
returning user_id, post_id, data, created_at, updated_at
`
...
select user_id, post_id, data, created_at, updated_at
from post
where user_id = $1 and post_id = $2
`
...
func (q *Queries) ReadPage(ctx context.Context, arg ReadPageParams) ([]Post, error) {
	rows, err := q.db.QueryContext(ctx, readPage, arg.UserID, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
